---
description: "連載構造案から統合記事を作成する汎用ワークフロー（Phase 6: レビュー）"
---

# Phase 6: レビュー（汎用）

> 前: `/series-unified-visual` | 次: 完了
> 知見ベース: [workflow-insights.md](../../agents/knowledge/workflow-insights.md)

---

## Step 0: 知見の読み込み

// turbo
1. 知見ファイルの確認:
   ```bash
   grep -A 10 "series-unified-review" agents/knowledge/workflow-insights.md 2>/dev/null
   ```
2. 関連する知見を抽出し、今回のレビューに活かす
3. 特に「失敗パターン」に注目

---

## 前提条件

以下が完了済み：

- Phase 2: コード実装・テスト
- Phase 3: 原稿作成
- Phase 5: 挿絵生成・埋め込み

---

## Step 1: コード検証

### テスト再実行

// turbo
```bash
cd agents/tests/{SLUG} && prove -l t/
```

### 記事内コードとの一致確認

1. 記事内のコードブロックを抽出
2. テストディレクトリのコードと比較
3. 差異がある場合は同期

---

## Step 2: 構造確認

### 記事構造チェック

- [ ] 導入部（概要・対象読者など）はスキップし、フロントマター直後からいきなりストーリー等（第1章）から入っているか
- [ ] 各章の構成が統一（目標→動く→破綻→完成→ポイント）
- [ ] 章間の接続が滑らか
- [ ] まとめ章が全体を総括

### 目次確認（統合版）

- [ ] `toc: true` が設定されている
- [ ] 見出しレベルが適切（h2→h3→h4）
- [ ] 目次から各セクションにジャンプ可能

---

## Step 3: 文体・品質チェック

### 文体統一

- [ ] 敬体/常体が統一されている
- [ ] 技術用語の表記が統一されている
- [ ] コードコメントが日本語/英語で統一
- [ ] 記事内の太文字強調（`**text**`）は、1記事あたり1回（最重要箇所）のみになっているか

### 誤字脱字チェック

- [ ] 本文の誤字脱字
- [ ] コード内のコメント
- [ ] frontmatter（title, description）

---

## Step 4: frontmatter確認

```yaml
---
date: {正しい形式: YYYY-MM-DDTHH:MM:SS+09:00}
description: "{120文字以内、キーワード含む}"
draft: false
tags:
  - {関連タグ}
title: '{50文字以内、キーワード含む}'
toc: true
---
```

### チェックリスト

- [ ] dateが正しいフォーマット
- [ ] descriptionが120文字以内
- [ ] titleが50文字以内
- [ ] tagsが適切（言語、シリーズ、パターン）
- [ ] draft: false（公開準備完了時）

---

## Step 5: ビジュアル確認

### Hugoサーバーで確認

// turbo
```bash
hugo server -D -F
```

ブラウザで以下を確認：

- [ ] 記事が正しく表示される
- [ ] 画像が全て表示される
- [ ] Mermaid図がレンダリングされる
- [ ] コードブロックがシンタックスハイライトされる
- [ ] 目次が正しく機能する

---

## Step 6: 最終チェックリスト

### 必須項目

- [ ] 全テストがpass
- [ ] 記事が正しく表示される
- [ ] 画像が全て表示される
- [ ] Mermaid図がレンダリングされる
- [ ] frontmatterが正しい

### 推奨項目

- [ ] SEO最適化（title, description, tags）
- [ ] 読了時間が適切（目安: 5-15分）
- [ ] モバイル表示確認

---

## Step 7: ステータス管理ファイルの更新

### 7.1 SKRステータスの更新

1. 以下のコマンドを実行し、ステータスを `status:published` に変更する:

```bash
node ~/.agents/skills/semantic-knowledge-repository/scripts/save_knowledge.cjs "series-status-<slug>" '{"facts":["Title: <タイトル>","Structure File: agents/structure/<slug>.md","Article File: content/post/<YYYY>/<MM>/<DD>/<HHMMSS>.md","Publication Date: <YYYY-MM-DD>"],"keywords":["status:published","planning-status","<slug>"],"confidence_score":100,"summary":"Published article series: <タイトル>"}'
```

```markdown
| [<slug>.md](agents/structure/<slug>.md) | <タイトル> | <形式> | YYYY-MM-DD | [記事](/YYYY/MM/DD/HHMMSS/) |
```

---

## 完了

全てのチェックが完了したら、記事は公開準備完了です。

### 公開方法

1. **即時公開**: `draft: false` で保存、デプロイ
2. **予約公開**: `date` を未来日付に設定、デプロイ

### 成果物

| 種類 | 場所 |
|------|------|
| 記事 | `content/post/YYYY/MM/DD/*.md` |
| コード | `agents/tests/{SLUG}/` |
| 画像 | `static/public_images/YYYY/{SLUG}/` |

---

## 連載版の追加チェック

連載版（複数記事）の場合は以下も確認：

- [ ] 前回/次回リンクが正しい
- [ ] 目次記事から各回へのリンクが有効
- [ ] 公開日時が連続している
- [ ] 各記事のfrontmatterが統一されている

---

## コード探偵シリーズの追加チェック（該当する場合のみ）

本シリーズの場合は、最後に以下のキャラクター性・演出が守られているか確認する：

### フロントマター規約

- [ ] `categories: [tech]`（`Design Patterns` や `Perl` をカテゴリに入れない）
- [ ] 必須タグ: `design-pattern`, `perl`, `moo`, `{pattern名}`, `{アンチパターン名}`, `refactoring`, `code-detective`
- [ ] アンチパターン名タグが記事内容と一致している（例: God Class → `god-class`）
- [ ] 禁止: 日本語タグ、`oop` 等のシリーズ外タグ

### キャラクター・演出

- [ ] ワトソン君が「物分かりが良すぎない」こと（知識はあるが解決策のひらめきがなく、困惑・驚きのリアクションをとっているか）
- [ ] ワトソン君が読者の抱く「どうやって？」「なぜ？」という疑問を素直に代弁しているか
- [ ] ロックのセリフが「翻訳ミステリー調」であり、傲慢だが頼りになる探偵として描かれているか
- [ ] 「探偵の調査報告書」が記事末尾に所定のフォーマット（横長のテーブル）通りに配置されているか
- [ ] 「探偵の調査報告書」のテーブル内に `<br>` 等のHTMLタグが使用されていないか（Markdown標準のフォーマットのみ使用し、HTMLタグは一切禁止）

---

## Step 8: 知見の記録

今回のレビューで得た気づきを `agents/knowledge/workflow-insights.md` に追記:

```markdown
## YYYY-MM-DD: <コンテンツ名>

### ワークフロー: series-unified-review

### 知見タイプ: <成功/失敗/発見/フィードバック/改善>

**コンテキスト**: <状況の説明>

**知見**: <学んだこと>

**適用場面**: <今後どんな場面で活用できるか>
```

### 記録すべき典型例

- レビューで見つかった頻出問題
- 効率的なレビュー手順
- Hugoビルドの問題と解決策

> [!NOTE]
> 知見がない場合はこのステップをスキップ可能

