...
1. 記事本文を読み、**最も核心的な1シーン** を選定。まず回答の骨子（Skeleton）を計画し、それを複数の専門的視点から批判的に検証（Critique）した上で、自己省察（Reflexion）を経て再構築した『洗練された最終回答』を出力してください。
...
プロンプトはファイルとして保存してください。（agents/code-doctor-series/{slug}/primary-image-{ケバブケース名}.txt）
