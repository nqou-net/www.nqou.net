...
- [ ] パターン名は正式名称（日本語またはカタカナ）
- [ ] 医療メタファーと記事内容の整合性がある
- [ ] 40文字以内（SNS表示を考慮）

**NG例**:
- `コードドクター：Singletonパターン` → 【】で囲んでいない、メタファーなし
- `コードドクター【シングルトン】孤独なインスタンス` → `〜テーマ〜`形式でない
...
- [ ] **発言量**: 1回の発言は**1文以内**（連続で2文以上は差し戻し）
- [ ] **用語制限**: 技術用語**のみ**使用可（比喩・例え話は禁止）
- [ ] **口調**: ぶっきらぼう、無愛想、必要最小限
- [ ] **禁止パターン**:
  - 「〜ですね」「〜でしょう」などの丁寧語
...
- [ ] ドクターの発言**直後**に翻訳・補足がある
- [ ] **敬語必須**: 患者への発言は「〜ですね」「〜ですよ」「〜ましょう」（タメ口は禁止）
- [ ] メタファーを用いた説明をしている
- [ ] ドクターの代わりに詳しく説明する役割を担っている
