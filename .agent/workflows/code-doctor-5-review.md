...
- [ ] シリーズ外タグの混入禁止: `oop`, `programming` 等、シリーズ他記事に存在しないタグを入れない

**NG例（タグ）**:
- `tags: [command, perl, oop, code-doctor]` → `design-pattern` が欠落、シリーズ外の `oop` が混入
- `tags: [design-pattern, iterator, perl]` → `code-doctor` が欠落

### カテゴリ規約

- [ ] 英語・ケバブケース
- [ ] `tech` カテゴリに所属
- [ ] `design-pattern` をカテゴリに入れていないか（カテゴリは `tech`、`design-pattern` はタグ）

**NG例（カテゴリ・タグ混同）**:
- `categories: [design-pattern]` + `tags: [command, perl, code-doctor]` → カテゴリとタグが逆。正しくは `categories: [tech]` + `tags: [design-pattern, command, perl, code-doctor]`
- `categories: [design-pattern]` + `tags: [chain-of-responsibility, perl, oop, code-doctor]` → カテゴリとタグが逆＋`design-pattern`タグ欠落＋シリーズ外`oop`混入（3重エラー。実例: Chain of Responsibility記事）
...
- [ ] **呼称ルール**: 地の文で「助手」または「ナナコ」と呼ばれている（「女性が〜」「彼女は〜」は**禁止**）
  - **初登場時も例外なし**: 「女性——助手のナナコと名乗った——が柔らかく微笑む」のように初登場描写で「女性」を主語にするのもNG。「助手のナナコと名乗ったその人が、柔らかく微笑む」のように書き換える
...
- [ ] 禁止表現チェック:
  ...
  - ○「ドクターが〜と言った」（発言の引用はOK）
  - ○「〜のように見えた」「〜気がした」（推測はOK）
- [ ] **内心描写の日本語ニュアンス**: 翻訳調や不自然な言い回しになっていないか
  - ×「この人、今2文字も喋ったぞ」（日本語では文字数を数えるニュアンスが不自然）
  - 不自然な場合はカットするか、自然な驚き表現に書き換える
...
- [ ] 本文中の強調表現（`**重要**`）は**最も重要な1箇所だけ**に使用する
- [ ] 処方箋まとめのステップ名ボールドは構造的強調のため許容
- [ ] 複数箇所をボールドにしている場合、最も核心的な1箇所だけ残し他は解除する
  - **よくあるパターン**: ドクターの短い宣告（`「**責任の連鎖。**」`等）を全てボールドにしがち。診断名（症候群名）の宣告1箇所のみに絞ること（実例: Chain of Responsibility記事で7箇所→1箇所に修正）

### ノック音の描写

- [ ] ノック音は「コンコンコン」（3回）に統一する
