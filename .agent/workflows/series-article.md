...
// turbo
```bash
node ~/.agents/skills/semantic-knowledge-repository/scripts/search_knowledge.cjs "status:planning planning-status"
```

1. 連載構造案がSKR上に存在することを確認（`status:planning` のもの）
2. 該当がなければ「使用可能な構造案がありません。`/planning-v2` で構造案を作成してください」と案内
3. 採用する構造案（slug）を決定し、ステータスを `status:adopted` に更新する:
```bash
node ~/.agents/skills/semantic-knowledge-repository/scripts/save_knowledge.cjs "series-status-<slug>" '{"facts":["Title: <タイトル>","Structure File: agents/structure/<slug>.md"],"keywords":["status:adopted","planning-status","<slug>"],"confidence_score":100,"summary":"Adopted article series: <タイトル>"}'
```
...
全てのPhaseが完了したら、一連の流れを通じて得た知見を記録します：
