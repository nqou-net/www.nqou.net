...
エピソードの内容に応じて、以下の2モードから選択する。

| モード | 説明 | 選択基準 | 既存例 |
|--------|------|----------|--------|
| **A: 事務所シーン** | LCI事務所内のアイソメトリック図 | ワトソン君の来訪・人物の関係性がメインの場合 | 第1話（無人の事務所）、第2話（双子の来客） |
| **B: パターン概念図** | アンチパターン/デザインパターンを象徴的にアイソメトリックで視覚化 | パターンのメカニズム自体を印象づけたい場合 | 第3話（Observer塔と散弾鎖） |

> [!NOTE]
> どちらのモードでも「ダークネイビー × シアンアクセント」のカラーパレットは共通。
> 迷ったら **モードA（事務所シーン）** を選ぶとシリーズとして安定する。
