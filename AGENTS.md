...
- 太字（`**text**`）は極力使用しない。強調が必要な場合は見出しやコードスタン（`` ` ``）を活用する

# 品質レビュー基準

記事の品質レビュー時には、以下の観点を確認すること：

## 技術的正確性

- SOLID原則（特にSRP/OCP）への違反が適切に指摘されているか
- 破綻パターンで「なぜダメなのか」が具体的に説明されているか
- コード例が実際に動作可能か

## 構造案との整合性

- 調査ドキュメント（`content/warehouse/`）の内容が反映されているか
- 連載構造案（`agents/structure/`）のストーリー構成が守られているか
- 「動く→破綻→パターン導入→完成」の流れが明確か

## 表現スタイル

- 教科書的でない導入になっているか
- 遊び心のある表現が含まれているか（例：括弧書きでの補足、軽いツッコミ）
- 読者が「なるほど」と思える具体的な問題提起があるか
...
- [シリーズ記事一覧](SERIES.md) — 既存シリーズの重複チェック・内部リンク調査用
- [シリーズ記事のワークフロー](WORKFLOWS.md#シリーズ記事連載記事のワークフロー)
- [単体記事のワークフロー](WORKFLOWS.md#単体記事のワークフロー)
- [プロンプトテンプレート](WORKFLOWS.md#プロンプトテンプレート)
