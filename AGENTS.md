...
特定のタスク品質を標準化・効率化するための「スキル」を定義・運用する。

- **配置場所**: `.agent/skills/<skill-name>/`
- **必須ファイル**: `SKILL.md` (YAMLフロントマター + 具体的な手順/チェックリスト)
- **用途**: シリーズ記事の品質チェック（Reviewer）、複雑なタスクの自動化など
- **運用ルール**: 
  - 新しい定型業務が発生したらスキル化を検討する
  - スキル定義は定期的に見直し、改善する
...
| `agents/knowledge/` | **設定の信頼できる唯一の情報源（SSOT）**（キャラ設定、知見等） |
...
- 強調: `**太字**` を使用する場合は、必ず前後に半角スペースを入れる（例: ` **重要** `）。または「」で括って表現する
