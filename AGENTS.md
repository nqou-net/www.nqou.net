...
- ライター（Writer）：短いブリーフやアウトラインから下書きを生成します。
- 編集者（Editor）：可読性、語調、構成を改善し、文法を修正します。
- SEO アシスタント（SEO Assistant）：タイトル、メタ記述、見出し、キーワードを提案します。
- ローカライザー（Localizer）：必要に応じて別言語へ翻訳・適応します。
- フォーマッター（Formatter）：Hugo の front matter、ショートコード、コードブロック、画像を整えます。
- パブリッシュチェック（Publisher Checklist Agent）：公開準備が整っているか検証し、チェックリストを出力します。
...
この `AGENTS.md` を別トーンで追加したい、英語版も欲しい、または CI 用の JSON 出力テンプレートを生成してほしいなど要望があれば教えてください。
