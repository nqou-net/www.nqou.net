...
### このリポジトリでエージェントができること

- **ブログ記事の作成・編集**: 新しい記事を作成したり、既存の記事を編集できる
- **コードブロックの修正**: 記事内のコードブロックの言語指定を修正できる
- **front matterの更新**: 記事のメタ情報を更新できる
- **AGENTS.mdの改善**: このガイドライン自体を改善できる

**重要**: プロンプトに明示的なゴールが含まれていない場合でも、エージェントはリポジトリのコンテキストから「ブログ記事に関連する作業」として解釈します。例えば「○○について書いて」というプロンプトは、記事の作成として解釈されます。
...
このリポジトリはブログサイトのため、エージェントは曖昧なプロンプトを「ブログ記事の作成」として解釈します。

例：
- 「○○を作って」 → ○○に関する記事を作成
- 「○○について3つ案を出して」 → ○○に関する提案記事を作成
- 「○○を示すアバターを作って」 → アバターデザイン提案の記事を作成

**ベストプラクティス**: プロンプトには成果物の種類を明示することで、より正確な結果を得られます。
- 良い例：「○○についての記事を作成してください」
- 良い例：「○○のコードブロックを修正してください」

参考：[Copilot向けカスタムインストラクションを書くための5つのヒント](https://github.blog/ai-and-ml/github-copilot/5-tips-for-writing-better-custom-instructions-for-copilot/)
...
- 脚注セクションに「本記事はバイブ・ブロギングで作成しています」を含める
