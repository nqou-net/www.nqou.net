...
1. 企画・アイデア出し（creative-brainstormingエージェント）
2. 調査・情報収集（investigative-researchエージェント）
3. シリーズ（連載）のアウトラインの検討（各専門家エージェント）
4. 各記事については、単体記事のワークフローに従う。

### 単体記事のワークフロー
記事ごとのワークフローは以下の順とする。それぞれ専門家エージェントに割り当てる。

1. アウトライン作成（各専門家エージェント）
  - 推定文字数が 10000 字を超える場合、連載記事として再構成すること。
