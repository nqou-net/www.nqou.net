...
3. 連載構造案のSEO視点レビューと改善（search-engine-optimizationエージェント）
  ...
  - 回答案を作成した後、その内容を批判的にレビューし、論理的欠陥や曖昧な点をすべて修正した**最終稿**のみを出力すること。プロセスは内部で行い、結果だけを表示
...
2. 各記事の挿絵を追加する（illustration-craftspersonエージェント）
  ...
  - 推奨される図の種類（以下から適切なものを選択）:
    - **クラス図**: パターンの構造を示す（Proxy/RealSubjectの関係など）
    - **シーケンス図**: 処理の流れを示す（遅延初期化、アクセス制御分岐など）
    - **フローチャート**: 判断ロジックを示す（キャッシュヒット/ミスなど）
    - **構成図**: システム全体の関係を示す（ローカル/リモートの構成など）
  - 挿入位置は「パターンで解決する」セクションの冒頭を推奨
  - 図の挿入後は、前後のコードブロック開始タグが正しいことを確認する
...
1. シリーズ記事の公開日時を決定する
  ...
  - 第2回以降は、前回（第2回の場合は第1回）の**17秒後**とする
...
5. 各記事の品質レビュー（reviewerエージェント）
  - レビューは最低3回繰り返し、各回で以下を確認する：
    - **第1回**: 構造案との整合性確認（ストーリー構成、1記事1概念）
    - **第2回**: 表現スタイル改善（遊び心、具体的な問題指摘）
    - **第3回**: 最終仕上げ（SRP/OCP言及、全体の一貫性）
  - 各回での改善点を記録し、次回レビューに反映する
...
- キーワード、および関連トピックについて、最新かつ信頼性の高い情報を調査・情報収集し、ドキュメントにまとめること。調査結果と共に**要点**、**根拠**、**仮定**、**出典（URL、書籍名（ASIN、ISBN）など）**、**信頼度（1~10の10段階評価）**を示すこと
- 競合記事の分析を行うこと
...
- 各案は、常識的な枠組みを取り払い、非常に極端だが革新的なアプローチにすること
- 実現可能性よりも、インパクトと独自性を最優先すること
...
- ストーリー構成: 各記事が「動く→破綻→パターン導入→完成コード」の流れを持つこと
- SOLID原則への言及: 破綻パターンでSRP/OCP違反を具体的に指摘していること
- 表現スタイル: 教科書的でなく、遊び心のある表現が含まれていること
- 挿絵の完備: 各記事にMermaid図（クラス図、シーケンス図等）が含まれていること
- リンク構造: シリーズ記事間のリンクと目次記事へのリンクが設定されていること
- コード例の完全性: 破綻例と解決例の2つのコード例が含まれ、言語・バージョン・依存が明記されていること
