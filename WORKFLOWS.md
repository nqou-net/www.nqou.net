...
| ワークフロー | 説明 |
|---|---|
| [planning-v2](.agent/workflows/planning-v2.md) | 汎用プランニングワークフロー（自己改善機能付き） |
...
```
調査結果（agents/warehouse/<slug>.md）に基づき連載構造案3案（A/B/C）作成:
...
- **キャラクター整合性**: `agents/knowledge/` の設定（口調、関係性）を厳守
- **レビュースキル実施**: `.agent/skills/` 内の該当レビュースキルによるチェック必須
- ストーリー構成: 動く→破綻→パターン導入→完成コード
- SOLID原則言及: 破綻でSRP/OCP違反を具体的指摘
- 表現スタイル: 遊び心ある表現
- 挿絵完備: Mermaid図（クラス図、シーケンス図等）。**技術的正確性と視覚的理解を両立する**
