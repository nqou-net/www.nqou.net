...
is($success->scenario_name, 'Success', 'scenario_name works correctly');

my $output = $success->execute;
ok($output, 'Execute produces output');
like($output, qr/200 OK/, 'Output contains status');

my $not_found = NotFoundScenario->new;
is($not_found->scenario_name, 'NotFound', 'scenario_name works for NotFound');
