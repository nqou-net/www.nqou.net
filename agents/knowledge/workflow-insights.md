...
* **YAMLフロントマター文字列制限**: `title` 等にシングルクォーテーション（アポストロフィ等）を含める場合は必ず全体をダブルクォーテーション `"..."` で囲むこと（Hugoビルド時のパースエラー防止）。
...
* **記事の書き出し（導入部スキップ）**: 「概要」「対象読者」「技術スタック」などのメタ情報を含む導入部（Introduction）は作成せず、フロントマターの直後からいきなりストーリー（第1章など）に入る構造を強制する。
