...
* **強調表現の絶対制限**: 記事内の太字（`**text**`）は「最重要結論1箇所のみ」。テーブル構文やリストによる暗黙の太字生成にも注意し、レビュー時に厳密に排除。可読性維持のため前後に半角スペース必須（例: ` **text** `）。
...
* **テスト実行時確認**: `prove` 実行時、ディレクトリではなく直接テストファイル（`test.t` など）を指定しなければならないパターンがあるため、実行前に構成を確認すること。
