...
**コンテキスト**: 記事内の強調表現（太字）の可読性向上に関するユーザー指示。

**知見**: Markdownで強調表現（`**text**`）を使用する場合は、前後に半角スペースを入れることで可読性が向上する（例: ` **text** `）。これは `AGENTS.md` および全コンテンツに適用される重要ルール。

**適用場面**: すべての執筆ワークフロー。

## 2026-02-04: ウイスキーテイスティング・ノート（Bridge × Adapter × Proxy）

### ワークフロー: series-unified-visual

### 知見タイプ: 成功

**コンテキスト**: 構造パターン（Adapter, Bridge, Proxy）の挿絵生成。記事のテーマは「ウイスキーテイスティング」。

**知見**:
1. **複合テーマの表現**: 「ウイスキーのノート」という具体的テーマと、「青写真（Blueprints）」という技術的メタファーを組み合わせたヘッダー画像が、記事の「実用×理論」というトーンに完璧にマッチした。
2. **抽象概念の可視化**: Adapterを「四角と丸をつなぐスチームパンク機械」、Bridgeを「2つの浮島をつなぐ橋」として具体化することで、抽象的なパターンを直感的なイメージに変換できた。プロンプトでは「Isometric view」や「Steampunk style」のような具体的なスタイル指定が有効だった。

**適用場面**: 抽象度の高いデザインパターン記事では、無理にコードを図解するより、そのパターンの「役割」を物理的な機械や建築物に例えたメタファー画像の方が、記事のアクセントとして効果的である。

## 2026-02-05: タイムトラベル冒険ゲーム（Chain of Responsibility+State+Memento）
...
**知見**: 
- macOSの `sips` コマンドが権限や一時ファイル書き込みの問題で失敗（Error 13）する場合がある。
- その場合、`magick` (ImageMagick) の `mogrify` を使用すると安定して一括リサイズが可能。
- プロンプトに "pixel art style" を含めることで、ゲームの冒険感を統一的に演出できた。
