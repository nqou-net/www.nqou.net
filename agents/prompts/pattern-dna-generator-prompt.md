...
> このプロンプトは `code-doctor-series.md` ワークフローから呼び出され、
> パターン名のみの入力から完全な構造案パラメータを自動生成します。
...
### 呼び出し元
- `.agent/workflows/code-doctor-series.md` Step 1
...
パターン名を受け取ったら、以下のステップを実行:

### Step 1: 症状カタログから逆引き

入力されたパターン名に対応する症状を「症状カタログ」から取得し、
**3つの症状**を選択する。

### Step 2: テーマの自動決定

症状の組み合わせから、最も適切なテーマを自動選択。
以下の優先順位で決定:
1. 症状が最も深刻に現れやすい領域
2. コード例が書きやすい領域
3. 読者に馴染みやすい領域
...
→ 以下が自動生成される:
- 症状: インスタンス増殖症、グローバル状態散乱症、初期化競合症
- テーマ: DB接続管理 or 設定ファイル管理
- 患者: Webアプリ開発者
