...
{{< linkcard "/2026/moo-text-editor-07/" >}}
...
今回は、複数の操作を1つにまとめる**マクロ機能**を実装します。
...
これは**Compositeパターン**と呼ばれるデザインパターンの一種です。
...
`execute`メソッドは、登録されたすべてのコマンドを**順番に**実行します。
...
`undo`メソッドは、登録されたすべてのコマンドを**逆順に**取り消します。
...
`MacroCommand`が強力なのは、**MacroCommandの中にMacroCommandを入れられる**ことです。
...
これは**再帰的な構造**であり、Compositeパターンの特徴です。
...
次回は、これらの機能を統合し、**対話的に操作できる簡易エディタ**を完成させます。
