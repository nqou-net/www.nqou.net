...
package Handler::Role {
    ...
    # Template Method パターン: 共通処理と個別処理の分離
    sub handle ($self, $request) {
        # 1. 自身のチェックを実行。エラーがあれば即座にそれを返す
        if (my $error = $self->check($request)) {
            return $error;
        }
...
ミカが声を上げた。私も見ていて分かった。元のif文の塊が、各Handlerの `check` メソッドとして切り出されている。だが、まだ疑問が残る。
...
（……なるほど。じゃあ私とロックの関係も、壊れやすい1本の太い鎖なのかな。いや、勝手につながれた鎖か）
