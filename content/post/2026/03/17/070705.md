...
「濃厚なにおいだね。カプセル化の崩壊臭……いや、もっと根深い。これは**不法侵入**のにおいだ」
...
「しかも、RealSubject——つまり本物の `UserProfile`——は、自分が代理人に覆われていることを一切知らない。これが **Proxy パターン** だ」
...
「このコードは、渡されたオブジェクトが本物の `UserProfile` なのか、それとも `UserProfile::Proxy` なのかを **一切知らない** し、**一切気にしない**。どちらでも同じように動く」
...
「合鍵なしでは入れない、要予約の寿司屋があるんだ。つまり正規のインターフェース——電話予約——を通さなければ誰も入れない、実に美しい設計の店だよ。ランチ1回分、おごってもらおうか」

「……それはつまり、僕の財布の中身にProxy経由でアクセスしている、ということですか？」

「初歩的なことだよ、ワトソン君。紳士は他人の財布の中身を直接覗いたりはしない。ただ、適正なインターフェースを通じてお願いするだけさ」

（……結局、まんまと奢らされるのは変わらないんだけど）

と心の中で呟きながら、僕は寿司屋の予約番号をスマホで調べ始めた。
