...
第2回では、外部のINI形式の設定ファイルを読み込む `load_config` メソッドを追加しました。これにより、コードを変更せずに設定を切り替えられるようになりました。
...
次回は、この問題を解決する方法を学びます。アプリケーション全体で同じ設定インスタンスを共有するにはどうすればよいのでしょうか？お楽しみに！
