...
VendingMachineクラス（Context）は**変更不要**です！
...
- SoldOutStateクラスを**新規追加**
- DispensingStateを**1箇所だけ修正**（遷移先の追加）
- VendingMachineState（ロール）は**変更なし**
- VendingMachine（Context）は**変更なし**
- WaitingStateは**変更なし**
- CoinInsertedStateは**変更なし**
...
在庫がなくなると、SoldOutState に遷移し、すべての操作で「売り切れ」と表示されます。
