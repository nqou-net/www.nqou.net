...
title: '第4回-浅いコピーの罠！装備を持ったモンスターで問題発生 - mass-producing-monsters'
---
...
`clone()`がコピーするのは属性値そのものです。武器属性には`Weapon`オブジェクトへのリファレンス（参照）が入っています。浅いコピーでは、このリファレンスがそのままコピーされます。

つまり、`$dragon`と`$dragon2`は同じ武器オブジェクトを参照しているのです。
