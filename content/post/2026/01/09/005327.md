...
```mermaid
flowchart TD
    DataExporter["DataExporter<br/>(Context)<br/>(管理クラス)<br/>エクスポーターを保持し、処理を委譲"]
    ExporterRole["ExporterRole<br/>(Strategy)<br/>(Moo::Role)<br/>exportメソッドを要求"]
    CsvExporter["CsvExporter<br/>(ConcreteStrategy)<br/>実際の処理を実装"]
    JsonExporter["JsonExporter<br/>(ConcreteStrategy)<br/>実際の処理を実装"]
    YamlExporter["YamlExporter<br/>(ConcreteStrategy)<br/>実際の処理を実装"]

    DataExporter --has exporter--> ExporterRole
    ExporterRole --implements--> CsvExporter
    ExporterRole --implements--> JsonExporter
    ExporterRole --implements--> YamlExporter
