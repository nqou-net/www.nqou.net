...
date: 2026-01-27T18:39:00+09:00
draft: true
tags:
  - perl
  - moo
...
image: /public_images/2026/pixel-art-editor-header.png
---
...
ターミナルで色を表示するには、`Term::ANSIColor` モジュールを使います。これはPerlのコア標準モジュールなので、追加インストールは不要です（嬉しい）。
...
実行すると、ターミナルに色付きのドット絵が表示されます。これだけでも、ちょっとワクワクしませんか？
...
あっ、やってしまった…この瞬間、私たちは「Undo機能のありがたさ」を痛感します。
...
つまり、「機能追加時に既存コードを壊すリスク」が大幅に下がります（これ、じわじわ効いてきます）。
...
`Storable::dclone` を使ってディープコピーを行います。これ、めちゃくちゃ重要です。参照渡しのPerlで、配列を完全にコピーする頼もしい方法です。これを使わないと、「復元したはずなのに元に戻らない」という罠にハマります。
...
ぜひ実際に動かして、「時間を巻き戻す」体験を楽しんでください。ターミナルでドット絵を描くのは、思った以上に楽しいですよ（ハマりすぎ注意）。
