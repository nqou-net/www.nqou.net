...
- `LimitChecker`: 金額上限チェックの責任だけ
- `ExpiryChecker`: 有効期限チェックの責任だけ
- `BlacklistChecker`: ブラックリストチェックの責任だけ

金額チェックは `LimitChecker`、有効期限チェックは `ExpiryChecker` というように、責任が明確に分離されています。これにより、テストが容易で、理解しやすく、再利用可能なコードになりました。
...
残高チェッカーを追加したくなったら、新しいクラスを作るだけです。既存の `LimitChecker`、`ExpiryChecker`、`BlacklistChecker` は一切変更せず、新しいクラスを追加するだけで拡張できます。これが「拡張に対して開き、修正に対して閉じた」設計です。
