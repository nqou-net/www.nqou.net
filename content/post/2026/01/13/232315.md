...
復元機能を`Player`クラスに追加する理由は、カプセル化を維持するためです。
...
`items`は配列リファレンスなので、特に注意が必要です。ここは参照を持つデータ構造を扱う際の重要なポイントです。
...
もし上記のようにそのまま代入すると、`Player`と`Snapshot`が**同じ配列（同じ住所）**を共有してしまいます。

これの何が問題かというと、**復元した後でアイテムを使ったときに、スナップショットの中身まで消えてしまう**のです。

**悪い例の動作イメージ:**

1. 復元直後: `Player`も`Snapshot`も、同じ `ref_A`（中身は「薬草」）を見ている。
2. プレイヤーが薬草を使う（削除する）。
3. `ref_A`の中身が空になる。
4. `Snapshot`はまだ `ref_A` を見ているので、**スナップショットの薬草も消えている！**
...
1. 復元時: `Snapshot`の `ref_A` の中身（薬草）をコピーして、新しい `ref_B` を作る。
2. `Player` は `ref_B` を見る。
3. プレイヤーが薬草を使う（`ref_B`から削除）。
4. `ref_B` は空になるが、`ref_A`（スナップショット）は**無傷のまま**。
...
`Player`と`Snapshot`が独立した配列を持つことで、安全にゲームを進めることができます。
