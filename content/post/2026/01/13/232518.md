...
これらの問題を解決するために、セーブデータを管理する専門のクラスを作ります。それが`GameManager`です。
...
`GameManager`クラスは、デザインパターンの用語でCaretaker（管理者）と呼ばれる役割を担っています。
...
この書き方だと、すべての`GameManager`オブジェクトが同じ配列を共有してしまいます。
