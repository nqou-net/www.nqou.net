...
# 言語: perl
# バージョン: 5.36以上
# 依存: Mojo::DOM（Mojoliciousに含まれる）、Path::Tiny
...
# ニュースを取得・表示
sub scrape_news () {
    my $dom = fetch_html('./sample_news.html');
...
# 天気を取得・保存
sub scrape_weather () {
    my $dom = fetch_html('./sample_weather.html');
...
# メイン処理
my $news_dom = fetch_html('./sample_news.html');
my @news = extract_news($news_dom);
save_to_screen("ニュース見出し一覧", @news);

say "";

my $weather_dom = fetch_html('./sample_weather.html');
