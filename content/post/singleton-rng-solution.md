...
- **プライベートコンストラクタ**: 外部から `new()` できないようにする
- **クラス変数**: 単一インスタンスを保持
- **getInstance()メソッド**: インスタンスへのアクセスを提供
...
- **`//=` 演算子**: Perl 5.10以降で使える便利な演算子で、未定義の時だけ代入する
- **ファイルスコープの `my $instance`**: パッケージ外からは見えない真にプライベートな変数
- **遅延初期化**: 最初の `getInstance()` 呼び出し時に生成し、メモリ節約になる
...
- **予期しない副作用**: 別の場所でRNGを使ったら思わぬところに影響
- **リファクタリングの障害**: 依存関係が暗黙的なので変更が怖い
...
- **テスタビリティが低い**: モック化困難で状態共有される
- **グローバル状態**: 隠れた依存関係によりデバッグ困難
- **SOLID原則違反**: 保守性の低下
...
- **単一性が必須**: 本当に1つだけであるべき理由がある
- **グローバルアクセスが必要**: どこからでもアクセスする必要がある
- **リソースが高コスト**: 初期化が重いまたはメモリ消費が大きい
- **テストの重要度が低い**: ユニットテストをあまり書かない小規模プロジェクト
