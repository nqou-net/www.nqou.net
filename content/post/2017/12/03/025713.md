...
```
 ...
 ## 感想 楽だった。 多くのコンテナを使うようなものじゃなくても、 \`Dockerfile\` を上書きするように使えるのは便利。 \`docker build\` する度に、タグを入れるのは面倒だし、そうでなくても \`docker images\` で \`\` がたくさん生成されるのを見ると色々と正しく指定しなければいけないのかなと思ってくる。 まあ、一段落したら、全部止めて要らなさそうなやつは消しておくようにはしてます。 
