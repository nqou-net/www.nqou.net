...
この記事では、私が実際に遭遇したこのバグを題材に、Perlの乱数生成の仕組みと、なぜSingletonパターンが必要なのかを学んでいきます。問題の発見から原因の特定、そして次回での解決までのデバッグの旅を一緒に体験しましょう。
...
- `seed`属性でシード値を管理
- `BUILD`メソッドで`srand()`を呼び出し、乱数ジェネレーターを初期化
- `get_number`メソッドで指定範囲の乱数を生成
...
2. `srand()`はこのグローバルな乱数ジェネレーターのシード値を設定
3. `rand()`はこのグローバルな乱数ジェネレーターから次の値を取得
4. どのパッケージから呼び出しても、同じ内部状態を共有
...
- ユーザーAのセッションにユーザーBがアクセス可能になる
- セッションハイジャック攻撃のリスクが生じる
- 個人情報の漏洩につながる
...
- Perlの`rand()`と`srand()`はグローバルな状態を共有
