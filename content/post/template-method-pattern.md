...
異なるデータ形式（CSV、JSON、XML）を処理する際、ファイル読み込み、データ検証、変換、保存といった処理の流れは同じなのに、形式ごとに似たようなコードを書いてしまう
...
Template Methodパターンは、以下の3つの原則でこれらの問題を解決する
...
GoF（Gang of Four）の定義では、Template Methodパターンは次のように説明されている
...
Django Class-Based Views（CBV）も、Template Methodパターンを採用している
...
ETL（Extract, Transform, Load）処理やレポート生成でも、Template Methodパターンは威力を発揮する
...
Template Methodパターンを使うと、すべてのサブクラスで処理順序が統一される
...
Template Methodパターンは、この原則を自然に実現する
...
Template Methodパターンは継承に依存するため、以下の制約がある
...
抽象メソッドが多すぎると、サブクラスの実装負担が大きくなり、パターンのメリットが失われる
...
アルゴリズムの骨組み自体が頻繁に変わる場合、Template Methodは適さない
...
このような多次元のバリエーションには、BridgeパターンやStrategyパターンの方が適している

**ケース4：実行時に処理を切り替えたい**

Template Methodパターンは、コンパイル時に処理が確定する。実行時に動的に処理を切り替えたい場合は、Strategyパターンを使うべきです
...
以下の条件を**すべて**満たす場合、Template Methodパターンを選択する
