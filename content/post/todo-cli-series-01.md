...
第1回の目標は、**if-elsif分岐で動作する最もシンプルなTodo CLIアプリを作成する**ことです。まずは動くものを作り、その限界を確認しましょう。
...
- `todo add "タスク名"` でタスクを追加する
- `todo list` でタスク一覧を表示する
- `todo complete 番号` でタスクを完了（削除）する
...
- `@ARGV` からコマンドライン引数を取得する方法を学んだ
- if-elsif分岐でサブコマンドを振り分けた
- テキストファイルでタスクを永続化した
- シンプルだが拡張性に課題があることを確認した
