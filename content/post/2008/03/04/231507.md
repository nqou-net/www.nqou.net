...
description: "先ほど導入したBMQuoteプラグインのドキュメントを読んでみた"
draft: false
...
先ほど導入した[BMQuoteプラグインのドキュメント](http://labs.m-logic.jp/plugins/mt-bmquote/docs/mt-bmquote.html)を読んでみた。
変数の値を変更するだけで、拡張フォーマットでの引用ができるらしい。
が、そこで参考にしている「XHTMLでの引用方法」のリンクが切れていたので、検索してみた。

**[XHTMLでの引用方法](http://www.nagaitosiya.com/c/quotation.html)**
