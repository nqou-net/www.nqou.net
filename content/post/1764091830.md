...
さらに、HTMLからMarkdownへの変換精度を上げるため、以下のようにお願いしました。
...
エージェントが作成したプログラムを [PR #2](https://github.com/nqou-net/www.nqou.net/pull/2) で実行してみたところ、blockquote（引用）が変換されていないことに気づきました。そこで、HTMLからMarkdownへの変換には専用のツールを使うことを検討し、html2markdownの導入を依頼した経緯がありました。
...
しかし、オチがあります。そもそもMarkdownとして書いていた部分が、変換ツールによってHTMLとして解釈されてしまい、意図しない変換が行われた箇所がありました。その部分は諦めて手作業で直しました。
