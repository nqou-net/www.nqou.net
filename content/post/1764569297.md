...
- GitHub Copilot Enterprise（推奨）または GitHub Copilot Business
  - Coding Agent 機能は Enterprise で完全サポート
  - Business プランでは一部制限がある場合あり
- リポジトリへの書き込み権限
...
Copilot Coding Agent は Enterprise/Business プランに含まれており、追加の API 課金は発生しません。

ただし、GitHub Actions のランナー時間は通常通り消費されます。目安として：
...
無料枠（Public リポジトリ無制限、Private は 2,000分/月）を超過した場合は、通常の Actions 料金が適用されます。
...
A: Model Context Protocol (MCP) を使用して、外部ツールやデータソースと連携できます。

```json
// .github/copilot/mcp.json の例
{
  "mcpServers": {
    "database": {
      "command": "npx",
...
詳細は [MCP 公式サイト](https://modelcontextprotocol.io/) を参照してください。
