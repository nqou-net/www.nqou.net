...
Perl Advent Calendar 2024の2日目です。今日は、Perlの真骨頂とも言える「ワンライナー」について深く掘り下げていきます。
...
# 特定の時間帯のログだけを抽出
perl -nle 'print if /2024-12-02 0[89]:/' access.log
...
# 名前付きキャプチャ（Perl 5.10+）
echo "2024-12-02" | perl -nle 'print "$+{day}/$+{month}/$+{year}" if /(?<year>\d{4})-(?<month>\d{2})-(?<day>\d{2})/'
```
...
# 日時文字列をエポック秒に変換
echo "2024-12-02 00:00:00" | perl -MTime::Piece -nle 'print Time::Piece->strptime($_, "%Y-%m-%d %H:%M:%S")->epoch'
...
- [perlrun - perldoc.perl.org](https://perldoc.perl.org/perlrun) - コマンドラインオプションの公式ドキュメント
- [Minimal Perl](http://www.oreilly.com/catalog/miniperl/) - Tim Maher著、ワンライナーに特化した書籍
- [Perl One-Liners](https://catonmat.net/perl-one-liners-explained-part-one) - Peteris Krumins による解説シリーズ
- [perlreref](https://perldoc.perl.org/perlreref) - 正規表現クイックリファレンス
