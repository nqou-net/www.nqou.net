...
エディタ拡張は、カスタムエージェントで強化できます。GitHub 公式の awesome-copilot を活用すれば、プロジェクト特有のコーディング規約や技術スタックに最適化されたエージェントを作成できます。
...
## プレミアムリクエスト 118 件の使い方徹底分析 - 何に使った？何が無駄だった？
...
| モデル | マルチプライヤー | 300リクエスト枠で使える回数 | 備考 |
|--------|-----------------|---------------------------|------|
| GPT-4o、GPT-4.1 | 0× | 無制限 | プレミアム制限にカウントされない |
| Gemini 2.0 Flash | 0.25× | 1,200回 | 高速で低コスト |
| Claude 3.5 Sonnet | 1× | 300回 | 標準の消費レート |
| Claude Opus 4 | 10× | 30回 | 高度な推論が必要な場合のみ |
| GPT-4.5 | 50× | 6回 | 最も高コスト、慎重に使用 |
...
2. **GPT-4o を優先的に使用**: マルチプライヤー 0×（実質無制限）
...
### Q5. Claude Opus 4 や GPT-4.5 などの高度なモデルはいつ使うべきですか？

高度なモデルはマルチプライヤーが高く（Claude Opus 4 は 10×、GPT-4.5 は 50×）、プレミアムリクエストを大量に消費します。以下のような場合にのみ使用を検討してください。

- 非常に複雑な問題解決が必要な場合
- 他のモデルでは対応できなかった場合
- プロジェクトの重要な意思決定に関わる場合

通常の開発業務では、GPT-4o（マルチプライヤー 0×、実質無制限）で十分です。
