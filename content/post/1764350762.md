...
この機能は「GitHub Copilot Coding Agent」と呼ばれています。従来のコード補完とは異なり、issueやタスクを非同期で自律的に処理してくれるエージェントです。

主な特徴は以下の通りです：

- **issueへの直接アサイン**: チームメイトにアサインするのと同じ感覚でCopilotをアサインできる
- **サンドボックス環境での実行**: GitHub Actionsを利用した安全な環境で動作
- **自動でPRを作成**: ドラフトPRを作成し、人間のレビューを待つ
- **レビューフィードバックへの対応**: PRへのコメントを受けて追加修正も可能

重要なのは、Copilotは自分でPRをマージすることはできないという点です。最終的な判断は必ず人間が行います。これにより、安全性を保ちながら自動化のメリットを享受できます（[GitHub公式ドキュメント](https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-coding-agent)）。
...
- [対象のissue](https://github.com/nqou-net/AAZe6Ev5fs2aCtcaginpy00jHimNhOmK/issues/9)
