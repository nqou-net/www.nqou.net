...
- 属性の組み合わせから新しい値を計算する
- オブジェクト作成時にIDや連番を振りたい
- 初期化時にログを記録したい

`DEMOLISH` を使う場面は以下の通りです。

- ファイルやデータベース接続などの外部リソースを解放する
- 一時的に作成したファイルを削除する
- 終了時にログを記録したい
...
- `BUILD` — オブジェクト作成直後に呼ばれる初期化メソッド
- `DEMOLISH` — オブジェクト破棄直前に呼ばれる後始末メソッド
- `BUILD` はIDの自動採番やログ記録に便利
- `DEMOLISH` は外部リソースの解放に使う

次回は最終回です。これまでに作ってきた掲示板システムを振り返り、次のステップについてお話しします。
