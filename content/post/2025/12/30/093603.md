...
- `use strict` と `use warnings` は、Perlのコードを安全に書くためのプラグマ
- `use feature 'say'` は、`print` に自動で改行を付ける `say` 関数を有効化する
...
```mermaid
flowchart TB
    subgraph Types["オプションの種類"]
        direction TB
        Flag["🚩 フラグ型<br/>値なし（ON/OFF）"]
        Value["📝 値を受け取る型"]
    end
    subgraph FlagEx["例: フラグ型"]
        F1["--verbose"]
        F2["--help"]
    end
    subgraph ValueTypes["値の型"]
        Str["=s 文字列<br/>--output=file.txt"]
        Int["=i 整数<br/>--count=10"]
        Flt["=f 浮動小数点<br/>--ratio=0.5"]
    end
    Flag --> FlagEx
    Value --> ValueTypes
```
