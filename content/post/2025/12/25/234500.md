...
この記事は「**Perlで値オブジェクトを使ってテスト駆動開発してみよう**」シリーズの**第4回（全5回）**です。前回は、[Test2によるTDD実践とMethodName値オブジェクト](/2025/12/23/234500/)の実装を通じて、Red-Green-Refactorサイクルを体験しました。今回は、**複数の値オブジェクトを組み合わせた複合的な値オブジェクト**として、JSON-RPC 2.0のRequestとResponseを完全実装します。
