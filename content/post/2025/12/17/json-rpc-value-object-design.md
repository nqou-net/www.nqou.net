...
前回は値オブジェクトの基本概念を学んだ。今回は**実際のプロトコル仕様書から値オブジェクトを設計するプロセス**を学んでいく。題材として、シンプルで明確な仕様を持つ**JSON-RPC 2.0**を取り上げる。
...
私自身も過去にPerlでJSON-RPCライブラリを実装した経験がある。
...
この「**不正なデータは存在させない**」という思想は、値オブジェクトの設計思想と完全に一致する。

### 3. 不変性との親和性

JSON-RPCのメッセージは一度作成されたら変更されません。リクエストは送信され、レスポンスは返される。

この性質は、値オブジェクトの**不変性（Immutability）**という特性と自然に調和する。

### 4. 厳密なバリデーションルール

仕様には、各フィールドの型、必須/オプション、値の制約が明確に定義されている。

これらをそのまま値オブジェクトのバリデーションルールとして実装できる。

## JSON-RPC 2.0仕様の構造分析

それでは、JSON-RPC 2.0の仕様を詳しく見ていこう。仕様書を読み解きながら、値オブジェクトとして実装すべき要素を抽出していく。

### Request objectの必須/オプション要素

JSON-RPCのリクエストは、以下の構造を持つJSONオブジェクトである。
...
この**MUST**というキーワードが重要である。RFC 2119では、MUSTは「絶対的な要件」を意味し、違反すると仕様非準拠となる。
...
`result`と`error`は**排他的**である。両方が存在することは許されない。

これは、値オブジェクト設計において**型で表現すべき重要な制約**である。
...
これらのエラーコードも、列挙型（enum）として値オブジェクト化できる。
...
JSON-RPC 2.0の仕様から、以下の値オブジェクトを抽出できる。
...
この中で、最もシンプルなのは**JsonRpcVersion**である。常に固定値 `"2.0"` を持つため、値オブジェクトの入門として最適である。

### 不変性をどう担保するか

値オブジェクトの不変性は、Mooの`is => 'ro'`（read-only）を使って実現する。
...
不変性を保証するには、オブジェクト生成時にすべての属性を初期化する必要がある。

Mooでは、`required => 1`または`default`を使って、すべてのフィールドが必ず値を持つことを保証できる。
...
仕様書から制約を読み取る際は、**RFC 2119のキーワード**に注目する。
...
これらの制約は、コンストラクタまたは`isa`でバリデーションエラーとして実装する。
...
それでは、最もシンプルな値オブジェクト`JsonRpcVersion`を実装してみよう。
...
一見すると、固定値を値オブジェクトにすることは過剰に思えるかもしれない。しかし、以下のような場合を考えてみてほしい。
...
実装の最後に`__END__`以降でPODドキュメントを記述している。値オブジェクトの目的、使い方、仕様へのリンクを含めることで、保守性が向上する。
...
特に重要なのは、**仕様書が値オブジェクト設計の最良のガイド**であるという点である。

RFC 2119準拠の仕様書は、そのまま実装の要求仕様として読み解ける。

### 値オブジェクト設計のプロセス

今回実践したプロセスをまとめると、以下のようになる。
...
このプロセスは、JSON-RPC以外のあらゆる仕様から値オブジェクトを設計する際にも応用できる。

### 次回予告：Test2によるTDD実践

次回は「**Test2でTDDを実践しよう - 値オブジェクトのテスト戦略**」と題して、テスト駆動開発（TDD）の実践的なワークフローを学ぶ。
