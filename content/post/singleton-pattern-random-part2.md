...
2. **グローバルアクセスポイントを提供**：どこからでもそのインスタンスにアクセスできる
...
- **ログマネージャー**：アプリケーション全体で1つのログファイルに書き込む
- **設定管理**：設定ファイルは1度だけ読み込み、全体で共有する
- **データベース接続プール**：接続を効率的に使い回す
- **乱数ジェネレーター**：今回のケース！グローバルな状態を持つため
...
1. **`my $_instance`**：パッケージスコープで定義されているため、全てのメソッドから参照可能だが、外部からは直接アクセスできない

2. **`instance()`メソッド**：
   - クラスメソッドとして呼び出す（`RandomGenerator->instance`）
   - `$_instance //= $class->new(@_)`で初回のみ`new`が実行される
   - 2回目以降は既存のインスタンスを返すだけ

3. **`_generate_seed()`**：
   - `time()`だけでなく、プロセスID（`$$`）も組み合わせる
   - ビットシフト（`$$<<15`）とXOR演算（`^`）で良い分散を得る
   - これにより同じ秒内でも異なるプロセスなら異なるシード値になる

4. **`BUILD()`**：
   - インスタンス生成時に**1度だけ**呼ばれる
   - `srand()`もこの時点で**1度だけ**実行される
   - 以降、`instance()`を何度呼んでも`BUILD()`は実行されない
...
**インスタンスが1つだけであることを保証**：
- 今回のような「グローバルな状態を持つリソース」の管理に最適
- 複数のインスタンスが存在することによる問題を根本的に防ぐ
...
**リソースの節約**：
- 不要なインスタンスを作らない
- メモリ、ファイルハンドル、ネットワーク接続などを効率的に使える

**遅延初期化**：
- 実際に必要になるまでインスタンスを生成しない
- 起動時間の短縮につながる
...
- グローバル変数と同様の問題を持つ
- 隠れた依存関係を作りやすい
- テストが難しくなる場合がある（今回は`_reset_instance()`で対処）

**マルチスレッド環境での考慮**：
- Perlのスレッドは複雑で、追加の考慮が必要
- 今回の実装は単一スレッドを前提としている
- スレッドセーフな実装には`Thread::Semaphore`などが必要

**過度な使用はアンチパターン**：
- 「便利だから」という理由だけで多用すると、設計が複雑化
- 依存性の注入（DI）パターンの方が適切な場合もある
- 「神クラス」化のリスク

**テスタビリティ**：
- モックやスタブへの置き換えが難しい場合がある
- 今回のように明示的なリセット機能が必要になることも
