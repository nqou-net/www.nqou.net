...
$info_uri = qq([$info_uriName]($info_uriAddr).);
$info_mail = qq((C)1999 [$info_mailName](mailto:$info_mailAddr). All rights reserved.);

$info_copyright = "<address>n<font face=times>$info_uri
n$info_mail</font>
n</address>n";
...
sub printRefArray{
    ...
    Jprint(qq(<li>**[$dirname（$title）]($dir)**n));
    return if($#$ra_dir < 0);
    print "
n";
    foreach $file(@$ra_dir){
        ...
        if(ref($file)){
            ...
            my($filename) = $file =~ m|([^/]+)$|;
            Jprint(qq(<li>[$filename（$title）]($file)n));
        }
    }
    print "
n";
...
# 英字なら「printBodyHeader("<font face=times>Title String</font>");」がいい
    ...
    Jprint(qq(# <a name="top">$title</a>n<hr>n));
    printLinks();
    print "</div>n";
}
