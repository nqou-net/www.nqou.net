...
**問題点**
- 通知先が増えるたびに`register_user`メソッドの修正が必要
- テストが困難（全ての通知先をモック化する必要がある）
- 通知先の変更が元のコードに影響を与える
...
**After（疎結合）の利点**
- UserManagerは「Observerインターフェース」のみを知る
...
1. **初期化**: ClientがSubjectとObserverを生成する
2. **登録**: `attach`メソッドでObserverをSubjectに登録する
3. **状態変更**: Subjectの状態が変わる
4. **自動通知**: SubjectがすべてのObserverの`update`メソッドを呼び出す
5. **処理実行**: 各Observerが独自の処理を実行する
...
- `trigger`による自動通知（`set_data`メソッドが不要）
- 型制約による実行時エラーの検出
- メソッドチェーン対応でコードが簡潔
- カスタムコールバックで柔軟な処理が可能
...
- ログメッセージを複数の出力先（ファイル、コンソール、メール）に送信する
- 実行時に出力先の追加・削除が可能
- ログレベル（INFO、WARNING、ERROR）に対応する
...
SubjectとObserverが互いに具体的な実装を知らないため、一方の変更が他方に影響を与えない。
...
新機能追加時に既存コードを修正しない（閉じている）が、拡張には開かれている。

[Strategyパターン](/2025/12/30/164012/)と並んで、OOPの重要原則を実現するパターンです。

### メリット2：動的な関係構築

実行時にObserverの追加・削除ができるため、柔軟な動作制御が可能です。
...
Observerを`detach`せずに破棄すると、Subjectが参照を保持し続けるためメモリリークが発生します。
...
ObserverがSubjectの状態を変更すると、無限ループが発生します。
...
Observerへの通知順序が保証されないため、依存関係がある処理で問題が発生します。
...
Observer数が多い場合、1回の`notify`で全てのObserverが同期実行されるため遅延が発生します。
...
どのObserverが登録されているかコード上で追跡しづらくなります。
