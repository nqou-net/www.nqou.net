...
| Creator | ファクトリメソッドを宣言する抽象クラス |
| ConcreteCreator | ファクトリメソッドを実装し、ConcreteProductを返すサブクラス |
...
この図は、Factory Methodパターンの一般的なUML構造です。Creator（作成者）がProduct（製品）を生成し、具体的な生成処理はConcreteCreator（具体的な作成者）に委ねられます。
...
|-------------|-------------------------------|
| Product | ReportRole（requires 'generate'） |
| ConcreteProduct | MonthlyReport, WeeklyReport, DailyReport, QuarterlyReport |
| Creator | ReportGenerator（create_report()を宣言） |
| ConcreteCreator | MonthlyReportGenerator, WeeklyReportGenerator, DailyReportGenerator, QuarterlyReportGenerator |
...
classDiagram
    ...
    Context o-- StrategyRole : has（委譲）
    StrategyRole <|.. ConcreteStrategyA
    StrategyRole <|.. ConcreteStrategyB
```
...
classDiagram
    ...
    Creator <|-- ConcreteCreatorA : extends（継承）
    Creator <|-- ConcreteCreatorB : extends（継承）
```
