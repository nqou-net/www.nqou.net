...
### エージェント・ワークフローシステム
- `agents/knowledge/` — SSOT（知見の集約先）
- `agents/warehouse/` — 調査情報
- `agents/structure/` — 連載構造案
- `agents/prompts/` — プロンプトテンプレート
- `.agent/workflows/` — ワークフロー定義
- `.github/agents/` — エージェント役割定義

詳細は [AGENTS.md](AGENTS.md) と [WORKFLOWS.md](WORKFLOWS.md) を参照。
