...
2. **リサーチ** - websearch-nerd - 最新情報と信頼できる情報源を収集
3. **下書き生成** - Hugo 用の Markdown 下書きを作成
4. **編集・校正** - blog-editor-nerd - スタイルと品質の向上
5. **SEO 最適化** - メタ情報と公開準備
...
**必ず守ること：**
- フロントマターは YAML（`---`）形式で記述
- `date` や `iso8601` フィールドは含めないこと（自動生成される）
- 下書き中は `draft: true` を設定
- タグは英語小文字、複合語はハイフン（例: `object-oriented`）
- 見出しは ATX スタイル、トップレベルは `##`
- 本文は必ず H2（`##`）から開始し、H1 はフロントマターのタイトルから Hugo により自動生成される
...
# 注意: date と iso8601 は含めない（公開時に自動生成される）
---
```
...
1. **フロントマター検証**
   ...
   - [ ] `draft: true` が設定されている
   - [ ] タグは英語小文字、ハイフンで単語を連結
   - [ ] `date` や `iso8601` は原則として frontmatter に含めない（AGENTS.md の方針に従う）。特別な理由がある場合のみ、明確な根拠とともに追加すること。
...
- ❌ H1 を本文に含める → ✅ H2 から開始
- ❌ date や iso8601 フィールドを手動で追加 → ✅ 含めない（自動生成される）
